...
(function () {
  ...
  // ── Preload heavy gallery images (poster orbit) so they're cached
  //    by the time three.js boots — no white planes, no pop-in.
  (function preloadWorks() {
    var warm = function () {
      for (var i = 1; i <= 20; i++) {
        var n = i < 10 ? '0' + i : '' + i;
        var img = new Image();
        img.decoding = 'async';
        img.src = 'assets/works/w' + n + '.jpg';
      }
    };
  ...
  const faqs = [
    ...
    { cat: 'pay',   q: 'Есть гарантия, если результат не подойдёт?', a: 'Да. Работаю по согласованному ТЗ: если агент не делает то, о чём договорились — дорабатываю бесплатно до результата. Две итерации правок уже включены в стоимость.' },
    { cat: 'tech',  q: 'Бот умеет переводить звонок на живого человека?', a: 'Да. Сложный или нестандартный случай агент передаёт менеджеру — звонком, в Telegram или в CRM. Вы сами решаете, что считать «горячим» и куда это направлять.' },
    { cat: 'tech',  q: 'А если агент чего-то не знает или ошибётся?', a: 'Он не выдумывает. На незнакомый вопрос отвечает честно и переводит на человека или фиксирует заявку. Все диалоги логируются — слабые места видно сразу, в первый месяц поддержки подкручиваю промпты.' },
    { cat: 'tech',  q: 'Какой номер нужен для голосового агента?', a: 'Можно подключить новый (выдаю виртуальный / SIP) или ваш текущий через переадресацию. Существующий номер заведения остаётся прежним — звонки просто начинает принимать агент.' },
    { cat: 'scope', q: 'Можно начать с малого и расширять потом?', a: 'Да, так и советую. Сначала пилот на одну задачу — например, приём звонков. Убеждаемся, что работает, потом достраиваем брони, отзывы, интеграции. Платите по мере роста.' },
    { cat: 'scope', q: 'Чем вы лучше конструкторов ботов и других фрилансеров?', a: 'Конструкторы ломаются на нестандартных диалогах. Я делаю под вашу логику: реальные брони, интеграции, передача человеку. Веду проект один, без потерь между людьми, и знаю HoReCa изнутри.' },
  ];
  ...
  (function initTierAccordion() {
    ...
    function reset() {
      if (!mql.matches) {
        ...
        // on mobile, pre-open the featured tier so the expand affordance is obvious
        const feat = tiers.find(t => t.classList.contains('price-tier-feature'));
        if (feat && !feat._kfSeeded) { feat._kfSeeded = true; feat.classList.add('is-open'); }
      }
    }
    attach();
    reset();
